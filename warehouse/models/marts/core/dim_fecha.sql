...
-- fct_pedido necesita fechas hasta 2026-09. date_spine garantiza cobertura
-- completa y sin huecos del rango que el resto del warehouse realmente usa
-- (arranca en 2024 porque las facturas son de 2024).
with spine as (
...
select
    ...
    weekofyear(date_day)::smallint as numero_semana_iso,
    ...
    -- bronze.calendario.es_feriado es una columna huérfana, 100% vacía: no hay
    -- de dónde sacar feriados, así que queda en false (default del DDL legacy)
    false as es_feriado,
